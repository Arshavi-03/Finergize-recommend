...
# Import OpenAI properly (client-based API)
try:
...
class RecommenderService:
    ...
    def configure_openai(self):
        ...
        if not use_openai:
            self.logger.info("OpenAI integration disabled by configuration")
            self.has_api = False
            return
        ...
        if openai_api_key and OPENAI_AVAILABLE:
            ...
            if not OPENAI_AVAILABLE:
                self.logger.warning("OpenAI package not available. Advanced features will be limited.")
            else:
    ...
    def load_model(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info("Standard loading successful")
                ...
                self.logger.info("Custom unpickler loading successful")
            ...
            self.model = FinergizeRecommenderAgent()
    ...
    def generate_survey(self, user_context):
        ...
        try:
            ...
            if hasattr(self.model, 'question_templates'):
                return list(self.model.question_templates.values())
            else:
                self.logger.error("Could not access question templates")
                return []
    ...
    def recommend_features(self, responses):
        ...
        try:
            ...
            if self.has_api and self.openai_client:
                ...
                try:
                    ...
                    # Call OpenAI with the modern client
                    ...
                    try:
                        ...
                        self.logger.info("Successfully generated OpenAI recommendations")
            ...
            # If OpenAI integration failed or is not available, try using model directly
            self.logger.info("Using model's recommendation algorithm")
            if hasattr(self.model, 'recommend_features'):
                try:
                    return self.model.recommend_features(responses)
                except Exception as e:
                    self.logger.error(f"Error using model's recommend_features: {e}")
    ...
    def generate_fallback_recommendations(self, responses):
        ...
        if hasattr(self.model, 'generate_fallback_recommendations'):
            try:
                return self.model.generate_fallback_recommendations(responses)
            except Exception as e:
                self.logger.error(f"Error using model's fallback recommendations: {e}")
